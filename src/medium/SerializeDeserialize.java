...
public class SerializeDeserialize {
	...
	public String serialize(TreeNode root) {
		...
		while (!queue.isEmpty()) {
			...
			sb.append(",");
			if (node == null) {
				if (currCount < Math.pow(2, height)) {
					queue.offer(null);
					queue.offer(null);
				}
			} else {
	...
	public TreeNode deserialize(String data) {
		...
		recursivelyCreateTree(root, 0, nodes);
		return root;

	}

	private void recursivelyCreateTree(TreeNode root, int index, String[] nodes) {
		if (index < (nodes.length / 2) && root != null) {
			TreeNode left = nodes[index * 2 + 1].equalsIgnoreCase("null") ? null
					: new TreeNode(Integer.parseInt(nodes[index * 2 + 1]));
			TreeNode right = nodes[index * 2 + 2].equalsIgnoreCase("null") ? null
					: new TreeNode(Integer.parseInt(nodes[index * 2 + 2]));
			root.left = left;
			root.right = right;
			recursivelyCreateTree(left, index * 2 + 1, nodes);
			recursivelyCreateTree(right, index * 2 + 2, nodes);
		}
	}

	private int findDepth(TreeNode node, int height) {
		if (node == null)
			return 0;
		if (node.left == null && node.right == null) {
			return height;
		}
		return Math.max(findDepth(node.left, height + 1), findDepth(node.right, height + 1));
	}
